...
function add_hide_node($node){
	$node_list=get_cookie("hide_node");
	
	if($node_list!=null){			
		$node_list=strReplace($node+",","",$node_list);
		$node_list=$node_list+$node+",";	
	}else{
		$node_list=$node+",";	
	}
	set_cookie("hide_node",$node_list);
};

function remove_hide_node($node){
	$node_list=get_cookie("hide_node");
	if($node_list!=null){
		$node_list=strReplace($node+",","",$node_list);
	}
...
function tree_menu_init(){	
	if(get_cookie("hide_node")!=null){
		$hide_node=get_cookie("hide_node");
		$hide_node_list=$hide_node.split(",");
		for (var s in $hide_node_list) {
			if($hide_node_list[s].length>0){
				icon=$("a[node='"+$hide_node_list[s]+"']");
				tree_menu=icon.parent().find(".tree_menu");
				tree_menu.hide();
				icon.find("i").removeClass("icon");
				icon.find("i").addClass("icon-angle-down");
			};
		}
	};
}
